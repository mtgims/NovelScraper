 ...
 * [TtsModels.isModelReady] would accept.
 */
object KokoroDownloader {
    ...
    fun download(
        ...
        val finalDir = TtsModels.modelDir(engine)
        val cacheDir = appCacheDir()
        val filesDir = appFilesDir()
        val tmpTar = File(cacheDir, "tts-model.tar.bz2")
        val tmpDir = File(filesDir, "${finalDir.name}.tmp")
        try {
            ...
            TtsModels.cleanupOtherModels()
            tmpDir.deleteRecursively()

            // 1. download the archive.
            val req = Request.Builder().url(TtsModels.downloadUrl(engine)).build()
            ...
            onProgress(
                if (TtsModels.isModelReady(engine)) Progress.Done
